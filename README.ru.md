...
Публикация собственного скилла в ZeroMarket: скомпилируйте в WASM, загрузите `tool.wasm`, `manifest.json` и `SKILL.md` через страницу загрузки ZeroMarket. Используйте `zeroclaw skill new <name> --template <lang>` для создания нового проекта скилла из шаблона.
...
- Руководство по WASM-скиллам: [`docs/wasm-tools-guide.md`](docs/wasm-tools-guide.md)
